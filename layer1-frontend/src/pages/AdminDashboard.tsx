...
const StatCard = ({ label, value, icon, sub, subColor, iconBg }: { label: string; value: number | string; icon: React.ReactNode; sub?: string; subColor?: string; iconBg?: string }) => (
  <div style={{ background: 'var(--mb-surface)', border: '1px solid var(--mb-border)', borderRadius: '14px', boxShadow: 'var(--mb-sh-card)', padding: '20px' }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
      <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--mb-text-2)' }}>{label}</span>
      <div style={{ width: '34px', height: '34px', borderRadius: '9px', background: iconBg || 'var(--mb-primary-tint)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{icon}</div>
    </div>
    <div style={{ fontSize: '34px', fontWeight: 700, color: 'var(--mb-ink)', lineHeight: 1, fontFamily: 'var(--mb-mono)' }}>{value}</div>
    {sub && <div style={{ fontSize: '12.5px', color: subColor || 'var(--mb-text-3)', marginTop: '9px', display: 'flex', alignItems: 'center', gap: '5px' }}>{sub}</div>}
  </div>
);
    ...
    <div style={{ width: '100vw', height: '100vh', display: 'flex', fontFamily: 'var(--mb-font)', background: 'var(--mb-bg)', color: 'var(--mb-text)', overflow: 'hidden' }}>
      ...
      <aside style={{ width: '256px', flexShrink: 0, background: 'var(--mb-surface)', borderRight: '1px solid var(--mb-border)', display: 'flex', flexDirection: 'column', padding: '22px 16px' }}>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {navItems.map((item, i) => (
            <button key={i} onClick={item.action} style={{ display: 'flex', alignItems: 'center', gap: '13px', height: '46px', padding: '0 14px', borderRadius: '10px', background: item.active ? 'var(--mb-primary-tint)' : 'transparent', color: item.active ? 'var(--mb-primary-dark)' : 'var(--mb-text-2)', fontSize: '15px', fontWeight: item.active ? 600 : 500, border: 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--mb-font)', width: '100%', position: 'relative', transition: 'all .15s' }}>
              {item.active && <span style={{ position: 'absolute', left: '-16px', top: '11px', bottom: '11px', width: '3px', borderRadius: '0 3px 3px 0', background: 'var(--mb-primary)' }} />}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <header style={{ height: '68px', flexShrink: 0, background: 'var(--mb-surface)', borderBottom: '1px solid var(--mb-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--mb-ink)', lineHeight: 1.1 }}>{data?.facility_name}</div>
              <div style={{ fontSize: '12.5px', color: 'var(--mb-text-3)', marginTop: '2px' }}>HFR ID {data?.hfr_id}</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
            ...
            <button onClick={handleLogout} style={{ height: '40px', padding: '0 16px', border: '1.5px solid var(--mb-border-strong)', borderRadius: '10px', background: 'transparent', fontFamily: 'var(--mb-font)', fontSize: '14px', fontWeight: 600, color: 'var(--mb-text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <main style={{ flex: 1, padding: '28px 30px', overflow: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '22px' }}>
            <div>
              <h1 style={{ fontSize: '25px', fontWeight: 700, color: 'var(--mb-ink)', margin: '0 0 5px', letterSpacing: '-.01em' }}>Business overview</h1>
              <p style={{ fontSize: '14.5px', color: 'var(--mb-text-2)', margin: 0 }}>ABDM activity and incentive earnings for your lab.</p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ height: '40px', padding: '0 16px', background: 'var(--mb-surface)', border: '1.5px solid var(--mb-border-strong)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '9px', fontSize: '14px', fontWeight: 600, color: 'var(--mb-text)', cursor: 'pointer' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="4" y="5" width="16" height="16" rx="2.2" stroke="#5b6b82" strokeWidth="1.8"/><path d="M4 9h16M8 3v3M16 3v3" stroke="#5b6b82" strokeWidth="1.8" strokeLinecap="round"/></svg>June 2026<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke="#5b6b82" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </div>
              <button onClick={() => navigate('/patients/register')} style={{ height: '40px', padding: '0 18px', background: 'var(--mb-primary)', color: '#fff', border: 'none', borderRadius: '10px', fontFamily: 'var(--mb-font)', fontSize: '14px', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 12px rgba(20,86,184,.2)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '22px' }}>
            <StatCard label="Reports shared this month" value={stats.reports} sub="+18% vs May" subColor="var(--mb-success)" iconBg="var(--mb-primary-tint)"
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 3l7 3v5.5c0 4.4-3 7.9-7 9.5-4-1.6-7-5.1-7-9.5V6l7-3z" stroke="#1456b8" strokeWidth="1.8" strokeLinejoin="round"/><path d="M9 12l2 2 4-4" stroke="#1456b8" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>} />
            <StatCard label="Patients registered" value={stats.patients} sub="Linked to ABHA" iconBg="var(--mb-teal-tint)"
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="3.6" stroke="#0d9488" strokeWidth="1.9"/><path d="M5 19c0-3.5 3.2-5.4 7-5.4s7 1.9 7 5.4" stroke="#0d9488" strokeWidth="1.9" strokeLinecap="round"/></svg>} />
            <StatCard label="Estimated earnings" value={`₹${(stats.reports * 25).toLocaleString('en-IN')}`} sub="@ ₹25 per eligible txn" iconBg="var(--mb-success-tint)"
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M7 6h9M7 10h9M14 6c0 4-3 5-6 5l5 7" stroke="#1f8a5b" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>} />
            <StatCard label="Total orders" value={stats.orders} sub="All time" iconBg="var(--mb-warning-tint)"
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M5 6h14M5 6l1 13h12l1-13M5 6l-1-2.5" stroke="#b9770b" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"/></svg>} />
          </div>
          ...
          <div style={{ background: 'var(--mb-surface)', border: '1px solid var(--mb-border)', borderRadius: '14px', boxShadow: 'var(--mb-sh-card)', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px 14px' }}>
              ...
              <span style={{ fontSize: '13.5px', fontWeight: 600, color: 'var(--mb-primary)' }}>{txns.length} records</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1.6fr 1.8fr 1.3fr 0.9fr 0.9fr', padding: '10px 22px', background: 'var(--mb-bg-subtle)', borderTop: '1px solid var(--mb-border)', borderBottom: '1px solid var(--mb-border)', fontSize: '11.5px', fontWeight: 600, letterSpacing: '.04em', textTransform: 'uppercase', color: 'var(--mb-text-2)' }}>
            ...
            {txns.length === 0 ? (
              ...
              txns.map((t, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.1fr 1.6fr 1.8fr 1.3fr 0.9fr 0.9fr', padding: '11px 22px', borderBottom: i < txns.length - 1 ? '1px solid var(--mb-border)' : 'none', alignItems: 'center' }}>
                  <div style={{ fontSize: '13px', fontFamily: 'var(--mb-mono)', color: 'var(--mb-text-2)' }}>{t.id}</div>
                  <div style={{ fontSize: '14px', color: 'var(--mb-ink)', fontWeight: 500 }}>{t.patient}</div>
                  <div style={{ fontSize: '13.5px', color: 'var(--mb-text)' }}>{t.test}</div>
                  <div style={{ fontSize: '13px', fontFamily: 'var(--mb-mono)', color: 'var(--mb-text-2)' }}>{t.time}</div>
                  <div style={{ textAlign: 'right', fontSize: '13.5px', fontFamily: 'var(--mb-mono)', fontWeight: 600, color: t.incColor }}>{t.inc}</div>
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', height: '24px', padding: '0 10px', borderRadius: '999px', background: t.bg, color: t.fg, fontSize: '12.5px', fontWeight: 600 }}>
                      <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: t.fg }} />
                      {t.label}
                    </span>
                  </div>
                </div>
